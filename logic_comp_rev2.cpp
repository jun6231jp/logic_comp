...
public:
  ...
  int add()
  {
    ...
    //BitTable.emplace_back(vec(Width,0));
    return LineNum;
  }
  void write(int val){
    if(Address%Width==0)
    ...
    for(int i=0; i<Width; i++)
      {
        ...
        CompList[i]=DupSearchInt(SearchNum,TargetLine,i);
        if(CompList[i]==0)
          //rtn=0;
	  return 0;
      }
  ...
  int DupSearchInt(int target, int SelfLine ,int pos=0)//テーブル内の自分の行以外から同じ数が見つかれば1を返す
  {
    for(int i = 0 ; i < LineNum; i++)
      {
	if(i < SelfLine)
...
#pragma omp parallel for //並列でグループ化可能な行を検索
      for(int i = 0 ; i < TableList[TableNum-2].LineNum; i++)
        {
		  ...
		  if(CompList[i][j]==0)
		    CompList[i][j]=TableList[TableNum-2].Search(i,j,CompChk[i][j]); // 中でDupSearchInt
                }
            }
        }
      ...
      cout << "Table" << TableNum-1 << " : " << TableList[TableNum-1].LineNum << endl;
...
#pragma omp parallel for
  ...
  cout << "Duplication delete from Table" << TableNo << " : " << ValidLineNum << endl;
}
