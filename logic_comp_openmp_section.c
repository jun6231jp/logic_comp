...
static block_t dummy;
static int logic_comp_res_pre;
static int logic_comp_res_post;
static int logic_resize_res;
static int resize_blocknum;

void
dummy_cp (void)
{
  dummy = block_list.block[block_list.block_num-1];
}
...
logic_comp_n (int dim)
{
  ...
  struct_add ();
  charnum = strlen (dummy.table[0]);
  line = (char *) malloc (sizeof (char) * (charnum + 1));
  compn_temp = (char **) malloc (sizeof (char *) * 1);
  compn_temp_line = dummy.size - 1;
  compn_temp =
...
resize (int dim)
{
  ...
  printf ("Resize ");
  charnum = strlen (block_list.block[resize_blocknum - 1].table[0]) + 2;
  line = (char *) malloc (sizeof (char) * charnum);
  i_buff_len = 0;
  line_num = block_list.block[resize_blocknum - 1].size - 1;

  for (int i = 0; i < charnum - 1; i++)
    {
      if (block_list.block[resize_blocknum - 1].table[0][i] == ' ')
        {
          i_buff_len++;
        }
    }
  sprintf (line, "%s", block_list.block[resize_blocknum - 1].table[0]);
  ...
  for (i = 0; i < line_num; i++)
    {
      sprintf (line, "%s",
               block_list.block[resize_blocknum - 1].table[i]);
  ...
  line =
    (char *) realloc (line,
                      sizeof (char) *
                      (strlen
                       (block_list.block[resize_blocknum - 2].table[0]) +
                       1));
  for (int m = 0; m < block_list.block[resize_blocknum - 2].size - 1;
       m++)
    {
      sprintf (line, "%s",
               block_list.block[resize_blocknum - 2].table[m]);
  ...
  table_reset (resize_blocknum - 1);
  for (int i = 0; i < resize_temp_line; i++)
    {
      table_write (resize_temp[i], resize_blocknum - 1);
      table_add (resize_blocknum - 1);
    }
...
duplicate_detect (int dim)
{
  ...
  charnum = strlen (block_list.block[resize_blocknum - 2].table[0]) + 2;
  line = (char *) malloc (sizeof (char) * charnum);
  dup_temp = (char **) malloc (sizeof (char *) * 1);
  dup_list = (char **) malloc (sizeof (char *) * 1);
  line_num = block_list.block[resize_blocknum - 2].size;
  sprintf (line, "%s", block_list.block[resize_blocknum - 2].table[0]);
  strtok (line, " ");
  len = strlen (line);
  part = (char **) malloc (sizeof (char *) * dim);
  ...
  for (int i = 0; i < block_list.block[resize_blocknum - 2].size - 1;
       i++)
    {
      count = 0;
      for (int j = 0; j < dim; j++)
        {
          strncpy(part[j], block_list.block[resize_blocknum - 2].table[i] + (j * (len + 1)),len);
          part[j][len]='\0';
        }
      for (int i = 0; i < dim; i++)
        {
          ...
          if (br[i] != 2)
            {
              for (int j = 0;
                   j < block_list.block[resize_blocknum - 1].size - 1;
                   j++)
                {
                  if (strstr (block_list.block[resize_blocknum - 1].table[j], part[i]))
                    {
                      br[i] = 2;
                      break;
                    }
              ...
              if (br[i] != 2)
                {
                  for (int k = 0;
                       k <
                         block_list.block[resize_blocknum - 2].size - 1;
                       k++)
                    {
                      if (strstr (block_list.block[resize_blocknum - 2].table[k], part[i]))
                        {
                          br[i]++;
                        }
                    }
  ...
  if (dup_temp_line != 0)
    {
      for (int i = 0; i < dup_temp_line; i++)
        {
          table_write (dup_temp[i], resize_blocknum - 1);
          table_add (resize_blocknum - 1);
        }
    }
  printf ("end  table_%d : %d\n", dim, dup_temp_line);
...
main (int argc, char *argv[])
{
  ...
  if (res == 1)
    ...
    {
      logic_comp_res_pre = logic_comp_2 ();
      if (logic_comp_res_pre == 2)
        {
          dummy_cp();
          logic_comp_res_post=logic_comp_res_pre;
          logic_comp_res_pre = logic_comp_n (dim);
        }
      while (logic_comp_res_pre == 2){
        dummy_cp();
        logic_comp_res_post=logic_comp_res_pre;
        resize_blocknum=block_list.block_num;
...
#pragma omp section
          {
            if (logic_comp_res_pre > 0)
              {
                logic_comp_res_pre = logic_comp_n (dim*2);
              }
          }
#pragma omp section
          {
            if(logic_comp_res_post > 0)
              {
                logic_resize_res = resize (dim);
                if (logic_resize_res == 1)
                  {
