  ...
  if(TableNum < 2)//LUTから2グループを抽出
    {
      ...
      vec2 CompList;
      vec Forbidden;
      CompList.resize(OptLineNum);
      Forbidden.resize(OptLineNum);
      for(int i = 0; i < OptLineNum; i++)
        {
          CompList[i].resize(OptPatternLength);
  ...
  else //Nグループから2Nグループを抽出
    {
      ...
      vec3 CompChk;
      vec2 CompList;
      CompList.resize(TableLineNum);
      CompChk.resize(TableLineNum);
#pragma omp parallel for //並列でグループ化可能な行を検索
      for(int i = 0 ; i < TableLineNum; i++)
        {
          CompList[i].resize(OptPatternLength);
          CompChk[i].resize(OptPatternLength);
          for (int j = 0; j < OptPatternLength; j++)
            {
              CompChk[i][j].resize(TableWidth);
