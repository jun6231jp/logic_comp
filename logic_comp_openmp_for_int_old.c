...
struct_init (void)
{
  ...
  block_list.block[0].table_int[0] = (int *) malloc (sizeof (int) * global_group_size * 4);
...
struct_add (void)
{
  ...
  block_list.block[block_list.block_num - 1].table_int[0] = (int *) malloc (sizeof (int) *  block_list.block[block_list.block_num - 1].group_size * 4);
  for (int i = 0; i < block_list.block[block_list.block_num - 1].group_size * 4; i++)
    {
...
table_add (int tbl)
{
  ...
  block_list.block[bl_no].table_int[block_list.block[bl_no].size - 1] = (int *) malloc (sizeof (int) *  block_list.block[bl_no].group_size * 4);
  for (int i = 0; i < block_list.block[bl_no].group_size * 4; i++)
    {
...
table_reset (int tbl)
{
  ...
  block_list.block[bl_no].table_int[0] = (int *) malloc (sizeof (int) *  block_list.block[bl_no].group_size * 4);
  block_list.block[bl_no].current = 0;
  for (int i = 0; i < block_list.block[bl_no].group_size * 4; i++)
    {
      block_list.block[bl_no].table_int[block_list.block[bl_no].size - 1][i] = 0;
...
uniq (int dim)
{
  ...
  for (int i = 0; i < size; i++)
    {
      uniq_temp_int[i] = (int *) malloc (sizeof (int) * dim * 4);
      for (int j = 0; j < dim; j++)
        {
...
logic_comp_n (int dim)
{
  ...
  for (int i = 0; i < compn_temp_line; i++)
    {
      compn_temp_int[i] = (int *) malloc (sizeof (int) * dim * 4);
      for(int j = 0 ; j < dim ; j++)
        {
  ...
  for (int i = 0; i < compn_temp_line ; i++)
    {
      pattern_int_store[i] = (int**)malloc(sizeof(int*) * compn_temp_line);
      for (int j = 0 ; j < pattern_len ; j++)
        {
          pattern_int_store[i][j] = (int*)malloc(sizeof(int) * dim * 4);
        }
      pattern_int[i]=(int*)malloc(sizeof(int) * dim * 4);
      pattern_n_int[i]=(int*)malloc(sizeof(int) * dim * 4);
...
duplicate_detect (int dim)
{
  ...
  for (int i = 0; i < block_list.block[block_list.block_num - 2].size; i++)
    {
      br[i] = (int *) malloc (sizeof (int) * dim * 4);
      for (int j = 0; j < dim * 4; j++)
        {
          br[i][j] = 0;
        }
...
#pragma omp parallel for private(chk)
  ...
  for (int j = 0; j < block_list.block[block_list.block_num - 2].size ; j++)
    {
      dup_temp_int[j] = (int *)malloc(sizeof(int) * dim * 4);
    }
